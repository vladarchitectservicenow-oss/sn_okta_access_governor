# Phase 1: Discovery & Design — Okta Access Governor

## Scope
This document captures the requirements-gathering, architectural design, and planning phase for sn_okta_access_governor.
...
- `x_sn_okta_access_governor_config`: Application configuration and properties.
- `x_sn_okta_access_governor_scan_log`: Record of each scan execution (start, end, status, record count).
- `x_sn_okta_access_governor_report`: Generated reports with metadata (format, size, checksum).
- `x_sn_okta_access_governor_delta_watermark`: High-water marks for incremental scanning per table.
...
| R3 | Scope collision with other apps | Medium | Low | Unique prefix `x_sn_okta_access_governor`; validate before install |
